...
def assign_simulation_times_to_time_ranges_average(time_intervals, times, list_gillespie_data):
    ...
    transposed_array = np.transpose(np.array(averaged_data_for_each_run))
...
def arithmetic_means(dataframe):
    ...
    return ret
# TODO: Tests schreiben
def calculate_differences(series_measured, df_simulated_normalized_means):
    """Evaluates how much a simulations differs from measured data."""

    differences = pd.DataFrame(np.zeros(df_simulated_normalized_means.shape), columns=df_simulated_normalized_means.columns, index=df_simulated_normalized_means.index)
    for run in df_simulated_normalized_means:
        differences[run] = abs(df_simulated_normalized_means[run]-series_measured)
