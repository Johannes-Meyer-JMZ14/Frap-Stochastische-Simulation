...
class Simulated_Annealing():
	...
	def __init__(self):
		...
		self.species_complex = ["x","y","z","x_f","y_f","z_f"]  # x free, y loosely bound, z tight bound, _f fluorescent
		self.species_out = ["x_f","y_f","z_f"]  # species for output signal
		# loss side
		self.L_complex = pd.DataFrame({"reaction01":[1, 0, 0, 0, 0, 0],  # x - x_f
					"reaction02":[0, 1, 0, 0, 0, 0],  # y - x
					"reaction03":[0, 0, 1, 0, 0, 0],  # z - y
					"reaction04":[0, 0, 0, 1, 0, 0],  # x_f - außen
					"reaction05":[0, 0, 0, 0, 1, 0],  # y_f - x_f
					"reaction06":[0, 0, 0, 0, 0, 1],  # z_f - y_f
					"reaction12":[1, 0, 0, 0, 0, 0],  # x - y
					"reaction13":[0, 1, 0, 0, 0, 0],  # y - z
					"reaction14":[0, 0, 0, 0, 0, 0],  # außen - x_f
					"reaction15":[0, 0, 0, 1, 0, 0],  # x_f - y_f
					"reaction16":[0, 0, 0, 0, 1, 0]},  # y_f - z_f
					index = self.species_complex)
		# gain side
		self.R_complex = pd.DataFrame({"reaction01":[0, 0, 0, 1, 0, 0],  # x - x_f
					"reaction02":[1, 0, 0, 0, 0, 0],  # y - x
					"reaction03":[0, 1, 0, 0, 0, 0],  # z - y
					"reaction04":[0, 0, 0, 0, 0, 0],  # x_f - außen
					"reaction05":[0, 0, 0, 1, 0, 0],  # y_f - x_f
					"reaction06":[0, 0, 0, 0, 1, 0],  # z_f - y_f
					"reaction12":[0, 1, 0, 0, 0, 0],  # x - y
					"reaction13":[0, 0, 1, 0, 0, 0],  # y - z
					"reaction14":[0, 0, 0, 1, 0, 0],  # außen - x_f
					"reaction15":[0, 0, 0, 0, 1, 0],  # x_f - y_f
					"reaction16":[0, 0, 0, 0, 0, 1]},  # y_f - z_f
					index = self.species_complex)
		self.N_complex = self.R_complex - self.L_complex

		# number of fluorescent molecules after bleach
		self.startQuantities_complex = {"x":[13],"y":[220], "z":[40], "x_f":[0], "y_f":[0], "z_f":[0]}

		self.time_limit_complex = 1160.0  # see time axis of measured data
		self.runs_complex = 40
		# colours for plotting
		self.colours_complex = {"x":"brown", "y":"green", "z":"mediumblue", "x_f":"red", "y_f":"limegreen", "z_f":"cornflowerblue"}
		...
		self.storage_path = './Daten/SimulatedAnnealing/WL_Komplex/'
		self.track_data =[]

	# objective function
	def objective_function(self, parameters):
		if len(parameters) != len(self.L_complex.columns):
			raise IndexError('The number of parameters has to equal the number of reactions in the given reaction network.')
		constants = {reaction:parameters[i] for i, reaction in enumerate(self.L_complex.columns)}
		# TODO: delete print
		# print(constants)
		# simulate one or more Gillespie runs
		trajectories = Glp.monte_carlo_gillespie(constants, self.L_complex, self.N_complex, self.startQuantities_complex, runs=self.runs_complex, time_max=self.time_limit_complex)
		# list of tuples of gillespie times_list and added_quantities_list for output signal
		list_of_output_data = Glp.make_output_signal(trajectories, self.species_out)
...
def run_simulated_annealing():
	...
	bounds = asarray([[0.0, 1.0]]*len(obj.L_complex.columns))
	...
	print('Done!')
	print('eval(%s) = %f' % (best, score))

	return [obj,best,score,scores]

runner = run_simulated_annealing()
with open(runner[0].storage_path + 'SimulatedAnnealingObject_pickle_binary', 'bw') as config_file:
	pickle.dump(runner[0], config_file)
with open(runner[0].storage_path + 'SimulatedBest_pickle_binary', 'bw') as best_scores:
	pickle.dump(runner[1:], best_scores)

# line plot of best scores
pyplot.plot(runner[3], '.-')
pyplot.xlabel('Improvement Number')
pyplot.ylabel('Evaluation objective(c)')
pyplot.savefig(runner[0].storage_path + 'scores.png')