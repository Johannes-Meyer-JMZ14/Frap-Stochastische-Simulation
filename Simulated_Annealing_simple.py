...
class Simulated_Annealing():
	...
	def __init__(self):
		...
		self.species_simple = ["x","y"]  # x free, y loosely bound
		# loss side
		self.L_simple = pd.DataFrame({"reaction01":[0, 0],  # außen - x
					"reaction02":[1, 0],  # x - y
					"reaction11":[1, 0],  # x - außen
					"reaction12":[0, 1]},  # y - x
					index = self.species_simple)
		# gain side
		self.R_simple = pd.DataFrame({"reaction01":[1, 0],  # außen - x
					"reaction02":[0, 1],  # x - y
					"reaction11":[0, 0],  # x - außen
					"reaction12":[1, 0]},  # y - x
					index = self.species_simple)
		self.N_simple = self.R_simple - self.L_simple

		# number of fluorescent molecules after bleach
		self.startQuantities_simple = {"x":[0],"y":[0]}

		self.time_limit_simple = 1160.0  # see time axis of measured data
		self.runs_simple = 40
		# colours for plotting
		self.colours_simple = {"x":"red", "y":"forestgreen"}
		...
		self.storage_path = './Daten/SimulatedAnnealing/Einfache_Bindung/'
		self.track_data =[]

	# objective function
	def objective_function(self, parameters):
		if len(parameters) != len(self.L_simple.columns):
			raise IndexError('The number of parameters has to equal the number of reactions in the given reaction network.')
		constants = {reaction:parameters[i] for i, reaction in enumerate(self.L_simple.columns)}
		# TODO: delete print
		# print(constants)
		# simulate one or more Gillespie runs
		trajectories = Glp.monte_carlo_gillespie(constants, self.L_simple, self.N_simple, self.startQuantities_simple, runs=self.runs_simple, time_max=self.time_limit_simple)
		# list of tuples of gillespie times_list and added_quantities_list for output signal
		list_of_output_data = Glp.make_output_signal(trajectories, self.species_simple)
...
def run_simulated_annealing():
	...
	bounds = asarray([[0.0, 1.0]]*len(obj.L_simple.columns))
